...
impl<V: HashValue, K: HashKey, M: HashValue> HashDatabase<V, K, M> {
    ...
    pub fn new<P: AsRef<Path>>(path: P, key: Vec<u8>) -> Result<Self, Box<dyn Error>> {
    ...
    pub fn insert(&mut self, key: K, item: V) -> Result<(), Box<dyn Error>> {
    ...
    pub fn load_meta(&mut self) -> Result<(), Box<dyn Error>> {
    ...
    pub fn load(&mut self, key: &K) -> Result<(), Box<dyn Error>> {
    ...
    pub fn index_entries(&self) -> Result<Vec<PathBuf>, Box<dyn Error>> {
        let results: Vec<Result<PathBuf, Box<dyn Error>>> = WalkDir::new(self.root.clone())
            .into_iter()
    ...
    pub fn index_subdatabases(&self) -> Result<Vec<PathBuf>, Box<dyn Error>> {
        let results: Vec<Result<PathBuf, Box<dyn Error>>> = WalkDir::new(self.root.clone())
            .into_iter()
            .map(|p| Ok(p?.path().to_path_buf()))
            .collect();
    ...
    pub fn read_entry<EV: HashValue>(&self, key: &K) -> Result<EV, Box<dyn Error>> {
        let path_str = key.to_string();
        let path = self.root.join(path_str);
        let mut file = File::open(path)?;
        let mut invec = Vec::new();
    ...
    /// used for special exceptions, for any large quantity of this type create a different database
    pub fn write_entry<EV: HashValue>(&self, key: &K, value: &EV) -> Result<(), Box<dyn Error>> {
        let path_str = key.to_string();
        let path = self.root.join(path_str);
        let mut file = File::open(path)?;
    ...
    pub fn archive(&self) -> Result<Vec<u8>, Box<dyn Error>> {
        let archive = Vec::new();
